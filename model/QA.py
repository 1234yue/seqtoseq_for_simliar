...
import data_utilsn
import jieba
from BM_get_answer import BM_answer
import re
...
tf.app.flags.DEFINE_integer("from_vocab_size", 123551, "English vocabulary size.")
tf.app.flags.DEFINE_integer("to_vocab_size", 123551, "French vocabulary size.")
tf.app.flags.DEFINE_string("data_dir", "/home/yueshifeng/repos/whole_data_add_attention_batch_for_simlar/data", "Data directory")
tf.app.flags.DEFINE_string("train_dir", "/home/yueshifeng/repos/whole_data_add_attention_batch_for_simlar/models", "Training directory.")
...
def train():
  ...
  """
  data_pre = '/home/yueshifeng/repos/whole_data_add_attention_batch_for_simlar/data/'
  from_dev = data_pre + 'train_id_test.in'
  from_train = data_pre + 'train_id.in'
...
def decode():
  ...
  with tf.Session() as sess:
    ...
    data_model = BM_answer()

    # Load vocabularies.
    en_vocab_path = os.path.join(FLAGS.data_dir,'train_dict.in')

    en_vocab, rev_fr_vocab = data_utilsn.initialize_vocabulary(en_vocab_path)
    ...
    while sentence:
      ...
      token_ids = data_utilsn.sentence_to_token_ids(sentence, en_vocab)
...
def state_similar(sentence1, sentence2, model,sess):
    dis = 0
    model.batch_size = 1  # We decode one sentence at a time.
    en_vocab_path = os.path.join(FLAGS.data_dir, 'train_dict.in')
    en_vocab, rev_fr_vocab = data_utilsn.initialize_vocabulary(en_vocab_path)
    sentence1 = rm_pun(sentence1)
    sentence2 = rm_pun(sentence2)
    token_ids1 = data_utilsn.sentence_to_token_ids(sentence1, en_vocab)
    token_ids2 = data_utilsn.sentence_to_token_ids(sentence2, en_vocab)
    for i, bucket in enumerate(_buckets):
        if bucket[0] >= len(token_ids1):
            bucket_id = i
            break
    encoder_inputs, decoder_inputs, target_weights, _ = model.get_batch(
        {bucket_id: [(token_ids1, [])]}, bucket_id)
    state1, _, output_logits = model.step(sess, encoder_inputs, decoder_inputs,
                                          target_weights, bucket_id, True)
    for i, bucket in enumerate(_buckets):
        if bucket[0] >= len(token_ids2):
            bucket_id = i
            break
    encoder_inputs, decoder_inputs, target_weights, _ = model.get_batch(
        {bucket_id: [(token_ids2, [])]}, bucket_id)
    state2, _, output_logits = model.step(sess, encoder_inputs, decoder_inputs,
                                          target_weights, bucket_id, True)
    dis = cal_distance(state1, state2)
    return dis
def norm_state_similar(sentence1, sentence2, model,sess):
    dis = state_similar(sentence1, sentence2, model,sess)
    if dis >2000:
        dis = 2000
    return 1-(dis/2000)
...
def main(_):
    ...
    model = create_model(sess, True)
    '''
    print(state_similar('北京到上海','北京 到 上海 五天 之 行',model,sess))
    print(norm_state_similar('北京到上海', '北京 到 上海 五天 之 行', model, sess))
    print(state_similar('北京到上海', '北京 到 上海 车 需要 多少 时间', model, sess))
    print(norm_state_similar('北京到上海', '北京 到 上海 车 需要 多少 时间', model, sess))
    '''
    print('请输入第一个句子')
    sys.stdout.write("> ")
    sys.stdout.flush()
    ...
    while sentence1:
        ...
        dis = norm_state_similar(sentence1,sentence2, model, sess)
        print('这两个句子的相似度为',dis)
        print('请输入第一个句子')
        sys.stdout.write("> ")
        sys.stdout.flush()
        sentence1 = sys.stdin.readline()
